...
+ (void) MR_findAllInContext:(NSManagedObjectContext *)context completion:(void(^)(NSArray *bookArray)) completion;
{
	return [self MR_executeFetchRequest:[self MR_requestAllInContext:context] inContext:context completion:completion];
}
...
+ (void) MR_findAllSortedBy:(NSString *)sortTerm ascending:(BOOL)ascending inContext:(NSManagedObjectContext *)context completion:(void(^)(NSArray *results)) completion;
{
	NSFetchRequest *request = [self MR_requestAllSortedBy:sortTerm ascending:ascending inContext:context];
	
	return [self MR_executeFetchRequest:request inContext:context completion:completion];
}
...
+ (NSArray *) MR_findAllSortedBy:(NSString *)sortTerm ascending:(BOOL)ascending withPredicate:(NSPredicate *)searchTerm propertiesToFetch:(NSArray *) fetchs inContext:(NSManagedObjectContext *)context; {
	NSFetchRequest *request = [self MR_requestAllSortedBy:sortTerm
                                                ascending:ascending
                                            withPredicate:searchTerm
										propertiesToFetch:fetchs
                                                inContext:context];
...
+ (NSArray *) MR_findAllSortedBy:(NSString *)sortTerm ascendings:(NSArray *)ascendingArray withPredicate:(NSPredicate *)searchTerm inContext:(NSManagedObjectContext *)context
{
	NSFetchRequest *request = [self MR_requestAllSortedBy:sortTerm
											   ascendings:ascendingArray
                                            withPredicate:searchTerm
                                                inContext:context];
...
+ (void) MR_findAllSortedBy:(NSString *)sortTerm ascending:(BOOL)ascending withPredicate:(NSPredicate *)searchTerm inContext:(NSManagedObjectContext *)context completion:(void(^)(NSArray *results)) completion;
{
	NSFetchRequest *request = [self MR_requestAllSortedBy:sortTerm
                                                ascending:ascending
                                            withPredicate:searchTerm
...
+ (NSArray *) MR_findAllSortedBy:(NSString *)sortTerm ascending:(BOOL)ascending withPredicate:(NSPredicate *)searchTerm propertiesToFetch:(NSArray *) fetchs {
	return [self MR_findAllSortedBy:sortTerm
                          ascending:ascending
                      withPredicate:searchTerm
				  propertiesToFetch:fetchs
...
+ (NSArray *) MR_findAllGroupByProperty:(NSString *) pname functionName:(NSString *) cname withPredicate:(NSPredicate *) searchTerm{
	return [self MR_findAllGroupByProperty:pname functionName:cname withPredicate:searchTerm inContext:[NSManagedObjectContext MR_contextForCurrentThread]];
}

+ (NSArray *) MR_findAllGroupByRelationships:(NSString *) pname functionName:(NSString *) cname withPredicate:(NSPredicate *) searchTerm{
	return [self MR_findAllGroupByRelationships:pname functionName:cname withPredicate:searchTerm inContext:[NSManagedObjectContext MR_contextForCurrentThread]];
}

+ (NSArray *) MR_findAllGroupByProperty:(NSString *) pname functionName:(NSString *) cname withPredicate:(NSPredicate *) searchTerm inContext:(NSManagedObjectContext *)context {
	NSEntityDescription *entity = [self MR_entityDescriptionInContext:context];
	NSPropertyDescription *propDesc = [[entity propertiesByName] objectForKey:pname];
	NSExpression *propExpr = [NSExpression expressionForKeyPath:pname];
	NSExpression *countExpr = [NSExpression expressionForFunction:[NSString stringWithFormat:@"%@:",cname] arguments:[NSArray arrayWithObject:propExpr]];
	...
	[exprDesc setName:cname];

	NSFetchRequest *request = [self MR_requestGroupByArray:[NSArray arrayWithObject:propDesc]
												fetchArray:[NSArray arrayWithObjects:propDesc, exprDesc, nil]
											 withPredicate:searchTerm
												 inContext:context];
...
+ (NSArray *) MR_findAllGroupByRelationships:(NSString *) pname functionName:(NSString *) cname withPredicate:(NSPredicate *) searchTerm inContext:(NSManagedObjectContext *)context {
	NSEntityDescription *entity = [self MR_entityDescriptionInContext:context];
	NSRelationshipDescription *propDesc = [[entity relationshipsByName] objectForKey:pname];
	NSExpression *propExpr = [NSExpression expressionForEvaluatedObject];
	NSExpression *countExpr = [NSExpression expressionForFunction:[NSString stringWithFormat:@"%@:",cname] arguments:[NSArray arrayWithObject:propExpr]];
	NSExpressionDescription *exprDesc = [[NSExpressionDescription alloc] init];
	[exprDesc setExpression:countExpr];
	[exprDesc setExpressionResultType:NSInteger64AttributeType];
...
+ (NSArray *) MR_findAllSortedBy:(NSString *)sortTerm ascendings:(NSArray *)ascendingArray withPredicate:(NSPredicate *)searchTerm
{
	return [self MR_findAllSortedBy:sortTerm
                          ascendings:ascendingArray
                      withPredicate:searchTerm
...
+ (void) MR_findAllWithPredicate:(NSPredicate *)searchTerm inContext:(NSManagedObjectContext *)context completion:(void(^)(NSArray *results)) completion;
{
	NSFetchRequest *request = [self MR_createFetchRequestInContext:context];
	[request setPredicate:searchTerm];
