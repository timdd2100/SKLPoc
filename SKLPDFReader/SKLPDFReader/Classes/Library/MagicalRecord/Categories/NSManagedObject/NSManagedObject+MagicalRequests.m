...
+ (NSFetchRequest *) MR_requestAllSortedBy:(NSString *)sortTerm ascending:(BOOL)ascending withPredicate:(NSPredicate *)searchTerm propertiesToFetch:(NSArray *) fetchs inContext:(NSManagedObjectContext *)context {
	...
	[request setPropertiesToFetch:fetchs];
...
+ (NSFetchRequest *) MR_requestAllSortedBy:(NSString *)sortTerm ascending:(BOOL)ascending withPredicate:(NSPredicate *)searchTerm propertiesToFetch:(NSArray *) fetchs {
...
+ (NSFetchRequest *) MR_requestAllSortedBy:(NSString *)sortTerm ascendings:(NSArray *)ascendingArray withPredicate:(NSPredicate *)searchTerm inContext:(NSManagedObjectContext *)context
{
	NSFetchRequest *request = [self MR_requestAllInContext:context];
	if (searchTerm)
    {
    ...
    for (int i = 0 ; i < sortKeys.count ; i++)
    {
		...
		NSNumber *as = [ascendingArray objectAtIndex:i];
        NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:sortKey ascending:[as boolValue]];
        [sortDescriptors addObject:sortDescriptor];
    }
...
+ (NSFetchRequest *) MR_requestAllSortedBy:(NSString *)sortTerm ascendings:(NSArray *)ascendingArray withPredicate:(NSPredicate *)searchTerm;
{
	NSFetchRequest *request = [self MR_requestAllSortedBy:sortTerm
...
+ (NSFetchRequest *) MR_requestGroupByArray:(NSArray *) garray fetchArray:(NSArray *) farray {
	return [self MR_requestGroupByArray:garray fetchArray:farray inContext:[NSManagedObjectContext MR_contextForCurrentThread]];
}

+ (NSFetchRequest *) MR_requestGroupByArray:(NSArray *) garray fetchArray:(NSArray *) farray inContext:(NSManagedObjectContext *)context {
	NSFetchRequest *request = [self MR_createFetchRequestInContext:[NSManagedObjectContext MR_contextForCurrentThread]];
	[request setPropertiesToGroupBy:garray];
	[request setPropertiesToFetch:farray];
	[request setResultType:NSDictionaryResultType];
	return request;
}

+ (NSFetchRequest *) MR_requestGroupByArray:(NSArray *) garray fetchArray:(NSArray *) farray withPredicate:(NSPredicate *)searchTerm {
	return [self MR_requestGroupByArray:garray fetchArray:farray inContext:[NSManagedObjectContext MR_contextForCurrentThread]];	
}

+ (NSFetchRequest *) MR_requestGroupByArray:(NSArray *) garray fetchArray:(NSArray *) farray withPredicate:(NSPredicate *)searchTerm inContext:(NSManagedObjectContext *)context {
	NSFetchRequest *request = [self MR_requestGroupByArray:garray fetchArray:farray inContext:context];
	[request setPredicate:searchTerm];
	return request;
}
