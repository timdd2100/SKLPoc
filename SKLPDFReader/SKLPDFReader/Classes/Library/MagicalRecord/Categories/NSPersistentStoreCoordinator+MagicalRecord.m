...
#define kSqlliteDirectory @"Sqllite"
...
+ (NSPersistentStoreCoordinator *) MR_coordinatorWithAutoMigratingSqliteStoreNamed:(NSString *) storeFileName
{
	...
	NSError *error = nil;
	NSString *directPath = [NSString stringWithFormat:@"%@",[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES) objectAtIndex:0]];
	if(![[NSFileManager defaultManager] fileExistsAtPath:directPath]){
		if(![[NSFileManager defaultManager] createDirectoryAtPath:directPath withIntermediateDirectories:YES attributes:nil error:&error]){
			//DLog(@"%@",error);
		}
	}
	
    NSString *sqlliteDirectPath = [NSString stringWithFormat:@"%@/%@",directPath,storeFileName];
	NSURL *storeURL = [NSURL fileURLWithPath:sqlliteDirectPath];
...
//    }
	...
	/* for iCloud */
	if ([[NSFileManager defaultManager] fileExistsAtPath:sqlliteDirectPath]) {
        storeURL = [NSURL fileURLWithPath:sqlliteDirectPath];
	}
